...
function findContentContainer(){
    ...
    var countTotlaWordsOnPage = document.body.innerText.match(/\S+/g).length, 
        // 一篇網站內容中文字大多由 p 表示
    ...
    for (var i = 0; i < wordsContainers.length; i++){
        if (wordsContainers[i].offsetHeight !== 0){ //確認該內容是可被使用者閱讀的
            var containerInnerText = wordsContainers[i].innerText.match(/\S+/g); //與第四行使用同樣的比較法
        ...
        && selectedContainer.parentNode.innerText){ //這一圈裡必須要有字
            ...
            countSelectedWords = selectedContainer.innerText.match(/\S+/g).length;
        };
...
//console.log(DOMtoString(document))
